...
export default function ComparePage() {
  const { selectedIds, remove, toggle, clear } = useCompare();
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">Side-by-side comparison of {selectedTools.length} tool{selectedTools.length !== 1 ? "s" : ""}</p>
          </div>
          <button onClick={clear} className="rounded-lg border border-border px-4 py-2 text-xs font-medium text-charcoal-light hover:bg-navy-50 hover:text-navy transition-colors">
            Clear All
          </button>
        </div>
